...
const AdminCompanyDashboard = () => {
  ...
  const handleDeleteAgency = async (id: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/agencies/${id}`, { method: 'DELETE' });
      if (response.ok) {
  ...
  const handleEditAgency = (agency: Agency) => {
    ...
    setImagePreview(agency.image ? `http://localhost:5000/uploads/${agency.image}` : null);
    setImageFile(null);
    setIsEditing(true);
    ...
    <div className="flex min-h-screen">
      ...
      <div className="w-3/4 bg-gray-50 p-6">
        <div className="flex justify-between mb-6">
          <button onClick={() => setAddCompanyModalOpen(true)} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
            + Add Agency
          </button>
          <button onClick={handleBulkDelete} className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">
            Bulk Delete
          </button>
        </div>
            ...
            <tbody>
              {agencies.map((agency) => (
                <tr key={agency._id} className="border-b hover:bg-gray-50">
                  <td className="p-3">
                    <input type="checkbox" checked={selectedAgencyIds.includes(agency._id)} onChange={() => handleSelectAgency(agency._id)} />
                  </td>
                  <td className="p-3">
                    <Image src={agency.image ? `http://localhost:5000/uploads/${agency.image}` : '/placeholder-logo.png'} alt="Logo" width={40} height={40} className="object-cover" />
                  </td>
                  ...
                  <td className="p-3">
                    <button className="text-blue-500 hover:underline mr-4" onClick={() => handleEditAgency(agency)}>
                      Edit
                    </button>
                    <button className="text-red-500 hover:underline" onClick={() => handleDeleteAgency(agency._id)}>
                      Delete
                    </button>
                  </td>
                </tr>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-6 rounded shadow-lg w-full max-w-lg">
              ...
              <form className="space-y-4">
                ...
                ></textarea>
                <input type="file" accept="image/*" onChange={(e) => setImageFile(e.target.files?.[0] || null)} className="w-full p-2" />
                {imagePreview && (
                  <Image
                    src={imagePreview}
                    alt="Preview"
                ...
                <button type="button" onClick={handleAddOrEditCompany} className="w-full bg-blue-500 text-white py-2 rounded">
